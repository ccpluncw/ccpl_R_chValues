...
#' @param combFunY This specifies the combFun for the X distribution. If there are multiple items contributing to a single X distribution, this function describes how the values will be combined across items. The function must combine the rows of a matrix and return the result as a vector (e.g., rowMeans). The default just flattens the matrix into one large vector. DEFAULT = NULL
#' @param combFunY This specifies the combFun for the Y distribution. If there are multiple items contributing to a single Y distribution, this function describes how the values will be combined across items. The function must combine the rows of a matrix and return the result as a vector (e.g., rowMeans). The default just flattens the matrix into one large vector. DEFAULT = NULL
...
ch.batchOverlapFlex.old <- function(values, items, itemSet, df.combns, numRuns=1000, combFun = as.vector, combFunX = NULL, combFunY = NULL, outFile = NULL) {
  if(is.null(combFunX)) {
    if(is.null(combFun)) {
      stop("if combFun == NULL then both combFunX and combFunY must be specified. If either combFunX or combFunY is NULL then they both get set to combFun.")
    } else {
      combFunX <- combFun
    }
  }
  ...
  for(l in 1:(numCombinations - 1)){
    ...
    for(k in l:(numCombinations - 1)){
      ...
      pOut <- ch.distOverlapFlex(xValue,yValue,numRuns,combFunX = combFunX, combFunY = combFunY)
