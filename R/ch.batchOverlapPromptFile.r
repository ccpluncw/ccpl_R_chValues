...
ch.batchOverlapPromptFile <- function(values, items, df.prompts, itemAcolNames = "IA",itemBcolNames = "IB", numRuns = 1000, outFile = NULL, verbose = FALSE, combFun = ch.maxAveComb, ...) {
  ...
  for(l in 1:totalNumOverlaps){
    ...
    yValue <- list()
      ...
      alldat[l,] <- data.frame(itemAarray, itemBarray, averageP = pOut["percent"],sdp = pOut["sd"], overlap = pOut["overlap"],direction = pOut["direction"])
